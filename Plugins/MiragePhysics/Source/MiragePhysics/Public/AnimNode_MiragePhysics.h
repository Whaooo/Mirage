...
class UMiragePhysicsLimitsDataAsset;

#include "AnimNode_MiragePhysics.generated.h"
...
struct FBoneLimit
{
	...
	UPROPERTY(EditAnywhere, Category = LimitBase)
	FBoneReference Master = FBoneReference();
	UPROPERTY(EditAnywhere, Category = LimitBase)
	FBoneReference Follower = FBoneReference();
	bool IsMutual = false;
	...
	UPROPERTY()
		ECollisionLimitType Type = ECollisionLimitType::Bone;
...
struct MIRAGEPHYSICS_API FMiragePhysicsSettings
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (ClampMin = "0"), category = "MiragePhysics")
	float Damping = 0.1f;
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (ClampMin = "0"), category = "MiragePhysics")
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (ClampMin = "0"), category = "MiragePhysics")
	float Radius = 3.0f;
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (PinHiddenByDefault, ClampMin = "0"), category = "MiragePhysics")
	float LimitAngle = 0.0f;
};

USTRUCT()
struct MIRAGEPHYSICS_API FMiragePhysicsModifyBone
{
...
public:
	...
	UPROPERTY()
	TArray<int> LimitBones;

	UPROPERTY()
	FMiragePhysicsSettings PhysicsSettings;
...
USTRUCT(BlueprintType)
struct MIRAGEPHYSICS_API FAnimNode_MiragePhysics : public FAnimNode_SkeletalControlBase
{
	GENERATED_USTRUCT_BODY()
...
public:
	...
	UPROPERTY(EditAnywhere, Category = "Bones Limits")
	TArray< FBoneLimit> BoneLimits;

	UPROPERTY(EditAnywhere, Category = "Limits Data(Experimental)")
	UMiragePhysicsLimitsDataAsset* LimitsDataAsset = nullptr;
	...
	UPROPERTY()
	TArray< FMiragePhysicsModifyBone > ModifyBones;
...
private:
	...
	void CollectLimitBones();
	void CalcBoneLength(FMiragePhysicsModifyBone& Bone, const TArray<FTransform>& RefBonePose);
