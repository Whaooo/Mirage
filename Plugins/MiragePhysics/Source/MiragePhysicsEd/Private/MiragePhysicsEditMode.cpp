...
#define LOCTEXT_NAMESPACE "MiragePhysicsEditMode"

struct HMiragePhysicsHitProxy : public HHitProxy
{
	DECLARE_HIT_PROXY()

	HMiragePhysicsHitProxy(ECollisionLimitType InType, int InIndex, bool InFromDataAsset = false)
...
IMPLEMENT_HIT_PROXY(HMiragePhysicsHitProxy, HHitProxy);


FMiragePhysicsEditMode::FMiragePhysicsEditMode()
	: RuntimeNode(nullptr)
	, GraphNode(nullptr)
...
void FMiragePhysicsEditMode::EnterMode(UAnimGraphNode_Base* InEditorNode, FAnimNode_Base* InRuntimeNode)
{
	RuntimeNode = static_cast<FAnimNode_MiragePhysics*>(InRuntimeNode);
	GraphNode = CastChecked<UAnimGraphNode_MiragePhysics>(InEditorNode);
	...
	NodePropertyDelegateHandle = GraphNode->OnNodePropertyChanged().AddSP(this, &FMiragePhysicsEditMode::OnExternalNodePropertyChange);

	FMiragePhysicsEditModeBase::EnterMode(InEditorNode, InRuntimeNode);
}

void FMiragePhysicsEditMode::ExitMode()
{
	...
	FMiragePhysicsEditModeBase::ExitMode();
}

void FMiragePhysicsEditMode::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	...
	FMiragePhysicsEditModeBase::Render(View, Viewport, PDI);
}

void FMiragePhysicsEditMode::RenderSphericalLimits(FPrimitiveDrawInterface* PDI)
{
	if (GraphNode->bEnableDebugDrawSphereLimit)
	{
		for( int i=0; i< RuntimeNode->SphericalLimits.Num(); i++)
		{
			...
			if (Sphere.Radius > 0)
			{
				...
				PDI->SetHitProxy(new HMiragePhysicsHitProxy(ECollisionLimitType::Spherical, i));
		...
		for (int i = 0; i < RuntimeNode->SphericalLimitsData.Num(); i++)
		{
			...
			if (Sphere.Radius > 0)
			{
				...
				PDI->SetHitProxy(new HMiragePhysicsHitProxy(ECollisionLimitType::Spherical, i, true));
				DrawSphere(PDI, Sphere.Location, FRotator::ZeroRotator, FVector(Sphere.Radius), 24, 6,
...
void FMiragePhysicsEditMode::RenderCapsuleLimit(FPrimitiveDrawInterface* PDI)
{
	if (GraphNode->bEnableDebugDrawCapsuleLimit)
	{
		for (int i = 0; i < RuntimeNode->CapsuleLimits.Num(); i++)
		{
			...
			if (Capsule.Radius > 0 && Capsule.Length > 0)
			{
				...
				PDI->SetHitProxy(new HMiragePhysicsHitProxy(ECollisionLimitType::Capsule, i));
		...
		for (int i = 0; i < RuntimeNode->CapsuleLimitsData.Num(); i++)
		{
			...
			if (Capsule.Radius > 0 && Capsule.Length > 0)
			{
				...
				PDI->SetHitProxy(new HMiragePhysicsHitProxy(ECollisionLimitType::Capsule, i, true));
				DrawCylinder(PDI, Capsule.Location, XAxis, YAxis, ZAxis, Capsule.Radius, 0.5f* Capsule.Length, 25,
					GEngine->ConstraintLimitMaterialZ->GetRenderProxy(), SDPG_World);
				DrawSphere(PDI, Capsule.Location + ZAxis * Capsule.Length * 0.5f, Capsule.Rotation.Rotator(), FVector(Capsule.Radius),
...
void FMiragePhysicsEditMode::RenderPlanerLimit(FPrimitiveDrawInterface* PDI)
{
	if (GraphNode->bEnableDebugDrawPlanerLimit)
	{
		for (int i = 0; i < RuntimeNode->PlanarLimits.Num(); i++)
		{
			...
			PDI->SetHitProxy(new HMiragePhysicsHitProxy(ECollisionLimitType::Planar, i));
		...
		for (int i = 0; i < RuntimeNode->PlanarLimitsData.Num(); i++)
		{
			...
			PDI->SetHitProxy(new HMiragePhysicsHitProxy(ECollisionLimitType::Planar, i, true));
...
FVector FMiragePhysicsEditMode::GetWidgetLocation(ECollisionLimitType CollisionType, int Index) const
{
	if (!IsValidSelectCollision())
	{
...
FVector FMiragePhysicsEditMode::GetWidgetLocation() const
{
	return GetWidgetLocation(SelectCollisionType, SelectCollisionIndex);
}

bool FMiragePhysicsEditMode::GetCustomDrawingCoordinateSystem(FMatrix& InMatrix, void* InData)
{
	if (!IsValidSelectCollision())
	{
		return false;
...
FWidget::EWidgetMode FMiragePhysicsEditMode::GetWidgetMode() const
{
	FCollisionLimitBase* Collision = GetSelectCollisionLimitRuntime();
	if (Collision)
	{
...
FWidget::EWidgetMode FMiragePhysicsEditMode::FindValidWidgetMode(FWidget::EWidgetMode InWidgetMode) const
{
...
bool FMiragePhysicsEditMode::HandleClick(FEditorViewportClient* InViewportClient, HHitProxy* HitProxy, const FViewportClick& Click)
{
	bool bResult = FMiragePhysicsEditModeBase::HandleClick(InViewportClient, HitProxy, Click);

	if (HitProxy != nullptr && HitProxy->IsA(HMiragePhysicsHitProxy::StaticGetType()))
	{
		HMiragePhysicsHitProxy* MiragePhysicsHitProxy = static_cast<HMiragePhysicsHitProxy*>(HitProxy);
		SelectCollisionType = MiragePhysicsHitProxy->CollisionType;
		SelectCollisionIndex = MiragePhysicsHitProxy->CollisionIndex;
		SelectCollisionIsFromDataAsset = MiragePhysicsHitProxy->bFromDataAsset;
		return true;
	}
...
bool FMiragePhysicsEditMode::InputKey(FEditorViewportClient* InViewportClient, FViewport* InViewport, FKey InKey, EInputEvent InEvent)
{
	bool bHandled = false;
...
ECoordSystem FMiragePhysicsEditMode::GetWidgetCoordinateSystem() const
{
	return COORD_Local;
}

void FMiragePhysicsEditMode::OnExternalNodePropertyChange(FPropertyChangedEvent& InPropertyEvent)
{
...
bool FMiragePhysicsEditMode::IsValidSelectCollision() const
{
...
FCollisionLimitBase* FMiragePhysicsEditMode::GetSelectCollisionLimitRuntime() const
{
	if (!IsValidSelectCollision())
	{
		return nullptr;
...
FCollisionLimitBase* FMiragePhysicsEditMode::GetSelectCollisionLimitGraph() const
{
	if (!IsValidSelectCollision())
	{
		return nullptr;
...
void FMiragePhysicsEditMode::DoTranslation(FVector& InTranslation)
{
...
void FMiragePhysicsEditMode::DoRotation(FRotator& InRotation)
{
...
void FMiragePhysicsEditMode::DoScale(FVector& InScale)
{
...
bool FMiragePhysicsEditMode::ShouldDrawWidget() const
{
	if (IsValidSelectCollision())
	{
		return true;
...
void FMiragePhysicsEditMode::DrawHUD(FEditorViewportClient* ViewportClient, FViewport* Viewport, const FSceneView* View, FCanvas* Canvas)
{
	...
	FMiragePhysicsEditModeBase::DrawHUD(ViewportClient, Viewport, View, Canvas);
}

void FMiragePhysicsEditMode::DrawTextItem(FText Text, FCanvas* Canvas, float X, float& Y, float FontHeight)
{
...
void FMiragePhysicsEditMode::Draw3DTextItem(FText Text, FCanvas* Canvas, const FSceneView* View, FViewport* Viewport, FVector Location)
{
